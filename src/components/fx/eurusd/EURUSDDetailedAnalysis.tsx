...
export const EURUSDDetailedAnalysis = ({ conditionAnalysis, backtestComparisons }: Props) => {
  const [activeSubTab, setActiveSubTab] = React.useState<"conditions" | "backtests">("conditions");

  if (!conditionAnalysis) return <div className="p-20 text-center text-slate-500 font-bold uppercase tracking-widest text-[10px]">EUR/USD 分析データを同期中...</div>;

  return (
    <div className="space-y-6">
      <div className="flex gap-4 p-1 bg-slate-950/80 rounded-2xl border border-slate-900 w-fit mx-auto mb-6 shadow-2xl">
        <button
