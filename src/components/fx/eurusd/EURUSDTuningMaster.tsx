...
export const EURUSDTuningMaster = ({ config, drift, logs, onUpdate, onRefresh, onAnalyzeDrift }: Props) => {
  const [isUpdating, setIsUpdating] = useState(false);

  const handleModeChange = async (mode: FXTuningMode) => {
    setIsUpdating(true);
    try {
      await onUpdate({ mode }, `EUR/USD 調整モードを「${mode === "conservative" ? "保守" : mode === "aggressive" ? "積極" : "標準"}」に変更`);
      await onRefresh();
    } finally {
      setIsUpdating(false);
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-slate-100 uppercase tracking-widest flex items-center gap-3">
            <Settings2 className="text-indigo-500" /> EUR/USD 実戦運用チューニング
          </h2>
