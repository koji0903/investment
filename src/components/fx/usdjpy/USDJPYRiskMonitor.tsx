    ...
    <div className="p-6 bg-slate-900/50 border border-slate-900 rounded-[32px] space-y-6 relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between relative z-10">
        <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-400">運用ガバナンス</h3>
         </div>
         <div className="flex items-center gap-2">
            <span className="text-[9px] font-black text-slate-500 uppercase">システム状態:</span>
            <span className={cn(
              "text-[9px] font-black uppercase px-2 py-0.5 rounded",
              isCritical ? "bg-rose-500 text-white" : isWarning ? "bg-amber-500 text-white" : "bg-emerald-500 text-white"
            )}>
              {permission.status === "stop" ? "停止" : permission.status === "caution" ? "注意" : "正常"}
            </span>
         </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-800 relative z-10">
        <div className="flex items-center gap-3">
           <div className="w-10 h-10 rounded-xl bg-slate-950 flex flex-col items-center justify-center border border-slate-800">
              <span className="text-[8px] font-bold text-slate-500 uppercase">制限</span>
              <span className="text-sm font-black text-indigo-400">{permission.dailyTradeRemaining}</span>
           </div>
           <div className="space-y-0.5">
              <span className="text-[8px] font-bold text-slate-500 uppercase">実行可能</span>
              <p className="text-[10px] font-black text-slate-400 uppercase">当日残り取引数</p>
           </div>
        </div>
        
        <div className="flex items-center gap-3 justify-end">
           <div className="text-right space-y-0.5">
              <span className="text-[8px] font-bold text-slate-500 uppercase">実績</span>
              <p className="text-[10px] font-black text-slate-400 uppercase">勝率 {(metrics.winRate * 100).toFixed(0)}%</p>
           </div>
      ...
      {isCritical && (
        <div className="mt-2 p-3 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center gap-3">
           ...
           <p className="text-[9px] font-bold text-rose-400 leading-tight uppercase font-sans">
              {permission.reason}
           </p>
        </div>
      )}
