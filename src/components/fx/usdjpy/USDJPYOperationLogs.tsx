...
export const USDJPYOperationLogs = ({ simulations, violations }: Props) => {
      ...
      <div className="max-h-[500px] overflow-y-auto pr-2 space-y-4 custom-scrollbar">
        {activeTab === "trades" && (
          simulations.length === 0 ? (
            <div className="p-20 text-center text-slate-600 font-bold italic">履歴なし</div>
          ) : (
            simulations.map((sim) => (
              <div key={sim.id} className="p-4 bg-slate-950/40 border border-slate-900 rounded-3xl flex items-center justify-between group hover:border-slate-700 transition-all">
